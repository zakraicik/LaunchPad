...
export default function Navbar() {
  ...
  const isActive = (path: string) => {
    ...
    if (path.startsWith("/campaigns?") && router.pathname === "/campaigns")
      return true;

    // For nested routes (e.g., /campaigns/my, /account/contributions)
    if (path === "/campaigns/my" && router.pathname === "/campaigns/my")
      return true;
    if (
      path === "/account/contributions" &&
      router.pathname === "/account/contributions"
    )
      return true;
    if (
    ...
    if (
      ...
      router.pathname === "/admin/platform-admins"
    )
      return true;
    if (
      path === "/admin/fee-management" &&
      router.pathname === "/admin/fee-management"
    )
      return true;
  ...
  useEffect(() => {
    ...
    const isOnAccountPage = accountNavigation.some((item) =>
      isActive(item.href)
    );
  ...
  const shouldShowAdmin =
    isHydrated && isConnected && isAdmin && !isLoadingAdmin;
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <Link href="/" className="flex-shrink-0">
              <span className="text-xl font-bold text-blue-600 drop-shadow-sm">
                LaunchPad
              </span>
            </Link>
          </div>
            ...
            {navigation.map((item) => (
              <Link
                ...
                className={`text-gray-800 hover:text-blue-600 font-medium transition-colors relative drop-shadow-sm ${
                  isActive(item.href)
                    ? "text-blue-600 font-semibold after:absolute after:bottom-[-2px] after:left-0 after:right-0 after:h-0.5 after:bg-current"
                    : ""
                }`}
              >
                {item.name}
            ...
            {shouldShowAccount && (
              <div className="relative" id="account-dropdown">
                <button
                  ...
                  className={`inline-flex items-center text-blue-700 font-medium transition-colors hover:text-blue-900 ${
                ...
                >
                  ...
                  <div
                    className={`relative ${
                      accountNavigation.some((item) => isActive(item.href))
                        ? "font-semibold after:absolute after:bottom-[-2px] after:left-0 after:right-0 after:h-0.5 after:bg-current"
                        : ""
                    }`}
                  >
                    Account
            ...
            {shouldShowAdmin && (
              <div className="relative" id="admin-dropdown">
                <button
                  ...
                  className={`inline-flex items-center text-purple-700 font-medium transition-colors hover:text-purple-900 ${
