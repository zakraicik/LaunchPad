  ...
  useEffect(() => {
    function handleClickOutside(event: MouseEvent) {
      if (
        ...
        !addressPopoverRef.current.contains(event.target as Node)
      ) {
        setShowAddressPopover(false);
      }
      if (
        ownerPopoverRef.current &&
        !ownerPopoverRef.current.contains(event.target as Node)
      ) {
          ...
          {campaignAddress && (
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-1 md:gap-0 relative">
              ...
              <div ref={addressPopoverRef}>
                ...
                {showAddressPopover && (
                  <div className="absolute right-0 mt-2 w-[calc(100vw-2rem)] md:w-72 rounded-lg shadow-lg bg-white/90 backdrop-blur-xl ring-1 ring-black/5 z-10">
                    <div className="p-3 md:p-4">
