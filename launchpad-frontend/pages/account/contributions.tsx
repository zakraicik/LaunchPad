...
import {
  ...
  Campaign,
  ContributionEvent,
} from "../../hooks/campaigns/useContributions";
...
export default function UserContributions() {
  ...
  const campaignsForRefundCheck = useMemo(() => {
    return contributionEvents.map((event) => ({
      ...
      isRefundEligible:
        campaignStatuses[event.campaignId] === "Refund Eligible",
    }));
  ...
  const refundStatuses = useRefundStatuses(
    isHydrated && campaignsForRefundCheck.length > 0
      ? campaignsForRefundCheck
      : [],
  ...
  const stats = useMemo(() => {
    const uniqueCampaigns = new Set(
      contributionEvents.map((event) => event.campaignId)
    );
    ...
    const tokenStats = Object.entries(contributionsByToken)
        ...
        {contributionEvents.length > 0 ? (
                ...
                <ul role="list" className="-mb-8">
                  {filterContributions(contributionEvents).map((event) => {
                    ...
                    const isUnsuccessful =
                      campaign?.statusText === "Unsuccessful";
          ...
          <div className="text-center py-20">
            ...
            <p className="mt-2 text-sm text-gray-500 max-w-sm mx-auto">
              You haven't made any contributions yet. Discover exciting projects
              and start supporting innovators today!
            </p>
            <div className="mt-6">
              <Link
                href="/discover"
