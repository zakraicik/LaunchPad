...
const useRefundStatuses = (
  ...
  const campaignsToCheck = useMemo(
    () =>
      campaigns.filter(
        (c) => c.isRefundEligible && c.campaignAddress && userAddress
      ),
      ...
      campaignsToCheck.map((campaign) => ({
        ...
        queryFn: async () => {
          ...
          try {
            ...
            console.error(
              "Error checking if contributor has been refunded:",
              error
            );
            if (error.code !== "ACTION_REJECTED") {
              toast.error(
                error.message ||
                  "Failed to check if contributor has been refunded"
              );
            }
            return false;
          }
  ...
  return useMemo(
    () =>
      campaigns.reduce<Record<string, boolean>>((acc, campaign, index) => {
        ...
        const queryIndex = campaignsToCheck.findIndex(
          (c) => c.campaignId === campaign.campaignId
        );
        acc[campaign.campaignId] =
          queryIndex >= 0 ? refundQueries[queryIndex].data ?? false : false;
        return acc;
      }, {}),
